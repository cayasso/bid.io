...
Bid.prototype.set = function set(id, data, fn, remote) {
  console.log('======================== BID SET ======================', id, data);
  if (!id) {
  ...
  this.store.set(id, data, fn, remote);
  return this;
};
...
Bid.prototype.fetch = function fetch(id, fn) {
  console.log('======================== BID FETCH ======================', id);
  this.get(id, function get(err, bid) {
...
Bid.prototype.find = function find(query, fn) {
  ...
  console.log('======================== BID FIND ======================', query);
  query = query || {};
  //query.saleDate = query.saleDate || Date.today().toFormat('YYYYMMDD');
  this.store.find(query, function find(err, res) {
    console.log('BID RESULTS', err, res);
    if ('function' === typeof fn) fn(null, res);
  }.bind(this));
  return this;
};
...
Bid.prototype.update = function update(id, data, fn) {
  ...
  self.get(id, function get(err, bid) {
    ...
    if (err) {
      return self.error(err, fn);
    }

    if (!bid) {
      return self.error('Bid not found in cache', fn);
    }

    for (var key in data) {
      if ('undefined' !== typeof bid[key]) {
        bid[key] = data[key];
      }
    }
    return self.set(id, bid, function set(err, bid) {
